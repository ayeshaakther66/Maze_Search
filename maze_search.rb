...
class GameWindow < Gosu::Window
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      if (@columns[cell_x][cell_y].north != nil) and @columns[cell_x][cell_y].north.vacant and !@columns[cell_x][cell_y].north.visited
        @columns[cell_x][cell_y].north.visited = true
        north_path = search(cell_x,cell_y - 1)
      end

      if (@columns[cell_x][cell_y].south != nil) and @columns[cell_x][cell_y].south.vacant and !@columns[cell_x][cell_y].south.visited
        @columns[cell_x][cell_y].south.visited = true
        south_path = search(cell_x,cell_y + 1)
      end

      if (@columns[cell_x][cell_y].east != nil) and @columns[cell_x][cell_y].east.vacant and !@columns[cell_x][cell_y].east.visited
        @columns[cell_x][cell_y].east.visited = true
        east_path = search(cell_x + 1,cell_y)
      end
      if (@columns[cell_x][cell_y].west != nil) and @columns[cell_x][cell_y].west.vacant and !@columns[cell_x][cell_y].west.visited
        @columns[cell_x][cell_y].west.visited = true
        west_path = search(cell_x - 1,cell_y)
      end

      #pick one possible path if not 0
      if (north_path != nil)
        path = north_path
